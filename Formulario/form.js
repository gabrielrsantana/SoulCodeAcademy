...
class Validator {
  ...
  validate(form) {
    ...
    let inputs = form.getElementsByTagName('input');//pega todos elementos do input
    let selects = form.getElementsByTagName('select');

    // tranforma uma HTMLCollection -> array
    let inputsArray = [...inputs, ...selects];//retorna os iputs e selects e joga no inputarrays
    ...
    inputsArray.forEach(function (input) {
      ...
      for (let i = 0; this.validations.length > i; i++) {
        ...
        if (input.getAttribute(this.validations[i]) != null) {
          ...
          console.log("value no Inpustarray=",value);
  ...
  validateyear(input) {
    ...
    let idade = Math.round((dataAtual - dataUser) / 1000 / 60 / 60 / 24 / 365);//idade em milisegundos,ai tem que converter pra anos
